...
4. **Scope cap (hourly cadence — ship something visible every tick).** Target **2–3 epic packages or equivalent per tick**: up to ~25 files / ~900 changed lines, or ~25 lessons of semantic prose. Batch packages that touch the same surface (e.g. header chrome + theme toggle) so a surface is never rebuilt twice. The binding limit is not size but coherence: the deployed site must be coherent and gate-green after every push. If a batch can't reach coherence, ship the largest coherent subset and log the remainder.
5. **Implement.** For epic work packages: spawn the **Builder** with the package + spec sections and apply its patch (or implement directly if the package is trivial — one file, cosmetic). For non-epic items the orchestrator may implement directly. Match existing code style; consult the Style ledger.
   **Git is the orchestrator's alone.** Record `git rev-parse HEAD` BEFORE dispatching a Builder and tell it verbatim: no git commands, no commits, no pushes. After it returns, compare HEAD — if it moved, the Builder committed against instructions: run the full gate + screenshot + review chain retroactively on the resulting state, log the violation in the item's `log`, and (if the commit was also pushed, i.e. it already deployed) verify the live state rather than rewriting pushed history — never force-push to undo it.
6. **Verify — all gates must pass:**
   ...
   - ui: `node tools/audit-site.js --gate` + `node tools/checks/dom-smoke.js` + `check-nav-a11y.js` + `check-dark-tokens.js`
   - content: audit-lessons diff clean + `node tools/audit-site.js --gate` + `node tools/checks/check-beam-groups.js`
   - **Run every `tools/checks/*.js` before any push.** They are cheap and each one exists because something shipped broken. A check wired into nothing protects nothing.

   **Notation correctness outranks chrome.** `audit-lessons.js` only proves durations sum; it cannot see a beam, bracket, or stem that misstates the rhythm. When a package tick and a notation-correctness finding compete, notation wins — a lesson that teaches the wrong rhythm is worse than a page that looks dated.
...
6b. **Visual verification — any tick whose change affects rendered output.** Start the dev server in the background (`npm start`, poll `http://localhost:8080/` until it responds — never a blind sleep). Screenshot each affected page with headless Edge at desktop (1280) and, for layout-affecting changes, mobile:
   ...
   **Harness pages go under `_site/dev/`, never at the `_site` root.** `audit-site.js` walks every `.html` in `_site` and skips exactly one thing — a top-level `dev/` directory (line ~294). An iframe harness left at the root has no `<main>` and no skip link, so it fails `dom.pagesMissingMain` and `dom.pagesMissingSkipLink` and looks like a defect in the package under review. This has now cost two ticks. If a gate reports those two metrics rising together, suspect a leftover harness before you suspect the change, and **never** `--write-baseline` to clear it — that bakes a broken page into the committed baseline.
...
9. **Pacing — 60 minutes between ticks.** In-session timers (ScheduleWakeup, CronCreate) proved unreliable in this project; the working heartbeat is a **background sleeper task** whose completion notification wakes the next tick: `start=$(date +%s); sleep 3600; echo "elapsed $(($(date +%s)-start))s"` with `run_in_background`. Check the reported elapsed seconds — if the host cut it short, re-arm for the remainder instead of ticking early. Two consecutive empty discoveries → STOP and write a final PROGRESS summary.

**Budget.** Dylan's standing instruction (2026-08-05): do not run lean — always use the full panel. Only if the org spend limit *hard-blocks* an agent may the orchestrator verify directly; log `budget-lean` on that item so the weaker verification is visible, and restore the panel next tick. Self-review bias is the exact hazard the Reviewer exists to remove.
