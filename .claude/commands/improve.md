...
9. **End of tick — pacing hint for /loop:** more `ready` items remain → next wakeup ~120s. Backlog empty or everything parked → ~30 min. Two consecutive discovery passes produced zero new items → tell the loop to STOP and write a final summary at the top of PROGRESS.md.
